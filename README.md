...
- **Ни cookie, ни чужих трекеров, ни куки-баннеров.** Наружу — к чужим
  доменам — не уходит ничего вообще. На свой уходят счётчики нажатий:
  пустой `POST /e/<событие>` без тела, параметров, cookie и идентификатора,
  в ответ 204. В журнале остаётся домен, имя события и ноль байт; ни IP, ни
  User-Agent там нет физически, поэтому связать два события одного человека
  не по чему. Раньше здесь было написано «ноль трекеров» — после появления
  этих счётчиков это перестало быть правдой буквально, и формулировка
  заменена на то, чего действительно нет.
