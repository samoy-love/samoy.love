...
const IGNORED_URLS: RegExp[] = [
  ...
  // картинку или шрифт, должна валить тест по-прежнему. Отсюда якоря с ОБЕИХ
  // сторон: без ведущего ^ шаблон освобождал от проверки любой адрес любого
  // происхождения, лишь бы тот кончался на /e/<слово> — например
  // https://чужой.example/assets/e/img. Имя события — тот же шаблон, что у
  // nginx и заглушки (scripts/events-dev-endpoint.mjs).
  /^https?:\/\/[^/]+\/e\/[a-z][a-z0-9_]{2,39}$/,
];
